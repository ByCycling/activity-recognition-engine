...
def simplification(_input_df: pd.DataFrame) -> pd.DataFrame:
    ...
    _output_df = _output_df.set_index(
        pd.DatetimeIndex(_output_df["timestamp"].dt.tz_convert("UTC"))
    ).sort_index()
    ...
    _groups = _output_df.groupby(
        (_output_df["activity.type"] != _output_df["activity.type"].shift()).cumsum()
    )
    ...
    app.logger.debug(
        "{} type summary: {}".format(
            colored("[starting simplification]", "green"),
    ...
    for _index, _group in _groups:
        ...
        def simplify(replacement: str = None):
            ...
            log(
                text="changing {} to {} on index {}".format(
                    _current_type, replacement, _index
                ),
                tag="simplification",
                color="yellow",
            )
        ...
        def surrounded_by(_type: str):
            if _previous_type and _next_type:
                log(
                    text="{} preceded by previous: {} and followed next: {}".format(
                        _current_type, _previous_type, _next_type
                    ),
                    tag="surrounded",
                    color="blue",
                )
                ...
                log(
                    text="{} followed by next: {}".format(_current_type, _next_type),
                    tag="surrounded",
                    color="blue",
                )
                ...
                log(
                    text="{} preceded by previous: {}".format(
                        _current_type, _previous_type
                    ),
                    tag="surrounded",
                    color="blue",
                )
                ...
                raise ValueError(
                    "cannot compare surrounding value: no type definitions"
                )
        ...
        def log(tag: str, color: str, text: str):
            app.logger.debug(
                "{tag} {text}".format(tag=colored("[{}]".format(tag), color), text=text)
            )

        if (
            on_foot()
            and surrounded_by("in_vehicle")
            and _avg_speed > _max_avg_walking_speed
        ):
            log(
                text="walking surrounded by in_vehicle with average speed of {}".format(
                    _avg_speed
                ),
                tag="case",
                color="magenta",
            ...
            on_foot()
            and surrounded_by("on_bicycle")
            and _avg_speed > _max_avg_walking_speed
        ):
            log(
                text="walking surrounded by on_bicycle with average speed of {}".format(
                    _avg_speed
                ),
                tag="case",
                color="magenta",
